...
def parse_option():
    ...
    opt.model_name = '{}_{}_{}_{}_lr_{}_decay_{}_bsz_{}_temp_{}_trial_{}'.\
        format(opt.method, opt.dataset, opt.noise, opt.model, opt.learning_rate,
...
def set_loader(opt):
    ...
    if opt.noise > 0:
        ...
        train_dataset = DatasetWrapper(train_dataset, "symmetric", opt.noise)
        train_dataset.save_noise_labels(os.path.join(opt.save_folder, 'y.npy'))
...
def train(train_loader, model, criterion, optimizer, epoch, opt):
    ...
    for idx, (images, labels, not_noise) in enumerate(train_loader):
